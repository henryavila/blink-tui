...
import { blocksH } from '../glyphs/glyphs.js';
...
export interface ProgressBarProps {
  ...
  color?: string;
  /** Track (empty) colour. Defaults to `tokens.fgDim`. */
  trackColor?: string;
}
 ...
 * fractional eighth is dropped), like every other glyph that falls back.
 */
export function ProgressBar({ value, width, color, trackColor }: ProgressBarProps): React.ReactElement {
  ...
  const track = trackColor ?? tokens.fgDim;
  ...
  const empty = ' '.repeat(Math.max(0, width - usedCells));
